...
Definition conj_fact : forall P Q R, P /\ Q -> Q /\ R -> P /\ R :=
  fun P Q R H H0 => conj (proj1 P Q H) (proj2 Q R H0).
(** [] *)
...
Definition eq_cons : forall (X : Type) (h1 h2 : X) (t1 t2 : list X),
  ...
  fun _ h1 h2 t1 t2 HH HT =>
    match HH with
    | eq_refl h => match HT with
...
Definition curry : forall P Q R : Prop,
    ((P /\ Q) -> R) -> (P -> (Q -> R)) :=
  fun _ _ _ H HP HQ => H (conj HP HQ).
...
Proof.
  unfold propositional_extensionality. intros. apply H. split. 
...
Proof.
  ...
  assert (True = P). { apply (pe_implies_true_eq H). apply pf1. }
  destruct H0. destruct pf1. destruct pf2. reflexivity.
Qed.
(** [] *)
