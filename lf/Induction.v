...
Proof.
  ...
  - simpl. assert (forall a b, a * S b = a * b + a).
  { intros. induction a.
    - reflexivity.
    - simpl. rewrite IHa, <- plus_n_Sm, add_assoc. reflexivity. }
  rewrite H, IHn, add_comm. reflexivity.
Qed.
(** [] *) 
...
Proof.
  intros. induction n.
  - reflexivity.
  - simpl. rewrite IHn, add_assoc. reflexivity.
Qed.
...
Theorem add_shuffle3' : forall n m p : nat,
  n + (m + p) = m + (n + p).
Proof.
  intros. repeat rewrite add_assoc. 
  rewrite add_comm with n m. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. induction n.
  - reflexivity.
  - simpl. rewrite double_incr_bin, IHn. reflexivity.
Qed.
...
Proof.
  intros. induction b.
  - reflexivity.
  - simpl. rewrite <- IHb, <- nat_to_bin_double, double_plus, add_0_r. reflexivity.
  - simpl. rewrite <- IHb, <- nat_to_bin_double, double_plus, add_0_r. reflexivity.
Qed.
(** [] *)
